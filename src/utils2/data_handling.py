...
class ToTensor:
    def __call__(self, sample):
        # Convert inputs (RGB, Depth) to float tensors
        inputs = [torch.from_numpy(img.transpose((2, 0, 1))).float() for img in sample['inputs']]
        
        # FIX: Ensure target is a LongTensor containing class indices, not one-hot maps
        target = torch.from_numpy(sample['target']).long() 
        
        return {'inputs': inputs, 'target': target}

def calculate_mfb_weights(target_paths, classes):
    ...
    for cls_idx in classes:
        ...
        if cnt == 0 and tot == 0:
            frequencies.append(0.0001)  # Stability fallback
        else:
            frequencies.append(cnt / tot)

    # Median frequency balancing equation
